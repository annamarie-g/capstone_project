...
def tfidf_matrix(series):
    vectorizer = TfidfVectorizer(max_df = 0.25, min_df = 15, preprocessor = tp.custom_preprocessor, tokenizer = tp.custom_tokenizer, stop_words=stopwords.words('english'), lowercase=True)
...
def gradient_boosting(X_train, y_train): 
    gb = GradientBoostingRegressor(n_estimators = 400, verbose=True, max_features = 'sqrt', max_depth = 20, random_state=42)
...
def create_featurespace(df):
    ...
    df['total'] = df['total_text'] + ' ' + df['title'] 
    text_mat, text_features = tfidf_matrix(df['total'])
...
#    total_mat = reduce_dimensions(total_mat, n_topics=10000)
    ...
    print gb.score(X_test.toarray(), y_test) 
